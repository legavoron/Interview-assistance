...
Неуправляемые компоненты опираются на DOM в качестве источника данных и могут быть удобны при интеграции React с кодом, не связанным с React. Количество кода может уменьшиться, правда, за счёт потери в его чистоте. Поэтому в обычных ситуациях мы рекомендуем использовать управляемые компоненты.`
    ...
    },
    {
        ...
        a: `    e.preventDefault() - отключает стандартное поведение элемента по умалчанию. Например он отменит переход по ссылке
    ...
    {
        q: `Что такое псевдоэлементы и для чего они испольхуются?`,
    ...
    },
    {
        ...
        a: `Это корректоное отображение сайта в разных браузерах и на разных устройствах
    ...
    3. console.log(obj[prop1]) - верент значение свойства prop1, если оно есть, а если его нет, то вернет undefined`
    },
    {
    ...
    },
    {
        ...
        a: `    Пр наступлении события в браузере происходит всплытие. В результате обработчики вызываются сначала на самом вложенном элементе и дальше вызываются обработчики всех родителей элемента по цепочке
    ...
    },
    {
        ...
        a: `    Java Script использует механизм прототипного наследования, а это значит, что если свойство или метод не найде в текущем объекте, то поиск будет осуществляться дальшепо прототипам.
        
    Расширение нативного объекта - это значит добавление новых свойств его прототипу
    
    Если мы добавляем новое свойство прототипу, у нас могут возникнуть проблемы. Например, при использовании сторонних библиотек, в них может быть свойство с таким же названием и потом возникнет конфликт или что-то ьбудет работать некорректно`
    },
    {
    ...
    Но чтобы легче было отлавливать ошибки, которые в ES-5 были вызваны всплывынием переменных, создали temporal dead zone
    ...
    AJAX - 'это не технология сама по себе, а термин, который описывает подход к использованию нескольких существующих технологий для работы на стороне клиента и для создания оссинхронных web приложений
    
    С помощью AJAX web приложения могут отправлять данные на сервер и получать их с сервера ассинхронно
    
    А также с помощью AJAX страницы могут динамически изменять содержимое без перезвгрузки страницы.
    ...
    Этот принцип не позволяет Java Script выполнять запросы за границы домена. Это позволяет изолировать потенциально вредоносные документы. И не дает вредоносному сценарию на одной странице получить доступ к конфеденциальным данным на другой`
    },
    {
    ...
    },
    {
        ...
        a: `    HTMLCollection - это динамическая HTML коллекция, которая представляет собой массиво подобный объект дочерних элементов
    ...
    Это анологично тому, как если бы в момент применения метода использовался бы конструктор newString, например
    ...
    События которые оторые должны будут отработаться попадат в очередь событий. Когда стек освобождается, туда должно попасть следующее событие из очереди. Чтобы определить, какое именно событие из очереди попадет следующим в стек для выполнения, нужен event loop
    ...
    {
        q: `Что такое HTTP?`,
        a: `    HTTP - это прикладной протокол для передачи гипертекстовых докумеентов, по типу HTML
    ...
    HTTP - это протокол без сохранения состояния. Т.е. сервер не сохраняетникаких наших данных между парами запрос - ответ`,
        i: 'HTTP'
    },
    {
        q: `Из чего чтостоит HTTP запрос?`,
...
1. абстакция - отделение концепции от реализации
...
4. полиморфизм - возможность выполнить задачу разными способами`
    ...
    {
        q: `Что такое SOLID?`,
        a: `    Это аббрвиатура пяти принципов, которые используются в ООП
...
- принцип инверсий и зависимостей`
    ...
    Используется для нахождения кратных чиселю Число кратно другому числу, если делится на него без остатка
